...
const NoticeItem = ({ notice }) => {
  ...
  const [imageHeights, setImageHeights] = useState({}); // 이미지 높이 상태 관리
  const [currentIndex, setCurrentIndex] = useState(0);

  const handleImageLoad = (index, event) => {
    const { width: imageWidth, height: imageOriginalHeight } = event.nativeEvent.source;
    const calculatedHeight = (imageOriginalHeight * (width - 32)) / imageWidth;
    setImageHeights((prev) => ({ ...prev, [index]: calculatedHeight })); // 인덱스별로 height 저장
  };
    ...
    <View style={{ marginBottom: 16, paddingHorizontal: 16 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
        {notice.authorImage ? (
          ...
          <View
            ...
            <Text style={{ fontSize: 10, color: '#fff', textAlign: 'center' }}>프로필사진</Text>
          </View>
        )}
        <View style={{ flexDirection: 'column' }}>
          ...
          <Text style={{ fontSize: 12, color: '#8a8a8a' }}>{formattedDate}</Text>
        </View>
      </View>
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <Image
                ...
                style={{
                  width: width - 32,
                  height: imageHeights[index] || 200, // 각 이미지에 맞는 height 적용
                  borderRadius: 10,
                  backgroundColor: '#e9e9e9',
                  marginTop:16,
                }}
                resizeMode="cover"
                onLoad={(event) => handleImageLoad(index, event)} // 이미지 로드 시 높이 계산
              />
            )}
            ref={flatListRef}
            onScroll={(event) => {
              ...
              {notice.photos.map((_, index) => (
                <View
                  ...
                  style={{
                    ...
                    borderRadius: 4,
                    marginHorizontal: 4,
                    backgroundColor: currentIndex === index ? '#2ee8a5' : '#ccc',
                  }}
                />
              ))}
            </View>
      ...
      <View
        ...
        <TouchableOpacity
          ...
          {notice.isLiked ? (
            <Text style={{ fontSize: 12, color: '#ff6972' }}>좋아요 {notice.likes}개</Text>
          ) : (
            <Text style={{ fontSize: 12, color: '#000' }}>좋아요 {notice.likes}개</Text>
          )}
          {notice.isLiking && (
