...
const MypageMain = ({navigation}) => {
  ...
  const {
    data: parentsInfo = {},
    isPending: parentsInfoIsPending,
    isSuccess: parentsInfoIsSuccess,
  } = useParentsInfo();
  const logout = async () => {
    try {
  ...
  const StudentCard = ({student}) => (
    <TouchableOpacity
      ...
      onPress={() => {
        navigation.navigate('MypageStudentDetail', {student: student});
      }}>
    ...
    parentsInfoIsSuccess && (
      <View
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            navigation.navigate('MypageDetail', {parentsInfo: parentsInfo})
          }>
          ...
          <View style={{flex: 1, gap: 4}}>
            ...
            <View style={{flexDirection: 'row'}}>
              {parentsInfo.students.map((student, index) => (
                <Text
                  key={index}
                  style={[textStyles.R2, {color: colors.Gray06}]}>
                  {student.name}
                  {index === parentsInfo.students.length - 1 ? ' ' : ', '}
                </Text>
              ))}
              <Text style={[textStyles.R2, {color: colors.Gray06}]}>
                어머니
